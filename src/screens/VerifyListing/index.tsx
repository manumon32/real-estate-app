...
import {sendChat, sendVerificationDetails, uploadImages} from '@api/services';
...
const Chat = React.memo(({navigation}: any) => {
  ...
  const sendImageUrlToApi = (imageUrl: string): Promise<any> => {
    let payload = {
      ...
      senderType: 'user',
      message: 'Uploaded documents with more clarity.',
      files: [imageUrl],
    };
    updateVerificationDetails(payload);
    return sendVerificationDetails(payload, {
      token,
      clientId,
      bearerToken,
    });
              ...
              {verification_hasMore || verification_loading ? (
                <View style={styles.loadingContainer}>
                  <ActivityIndicator
                    ...
                    color={theme.colors.activityIndicatorColor}
                  />
                  {verification_loading && verificationDetails?.length > 0 && (
